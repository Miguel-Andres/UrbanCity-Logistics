...
^PW800
^LL1520
^FO50,300^CF0,60^FDURBAN CITY LOGISTICS^FS
^FO50,360^GB700,3,3^FS
^FO50,380^CF0,35^FDRostoReba^FS
^FO400,380^CF0,30^FDFecha: ${fechaFormateada}^FS
^FO50,420^GB700,1,1^FS
^FO50,440^CF0,30^FDNombre:^FS
^FO200,440^CF0,30^FD${datos.nombre}^FS
^FO50,475^CF0,30^FDTelefono:^FS
^FO200,475^CF0,30^FD${datos.telefono}^FS
^FO50,505^CF0,30^FDDireccion:^FS
^FO200,505^CF0,30^FD${datos.direccion}^FS
^FO50,535^CF0,30^FDLocalidad:^FS
^FO200,535^CF0,30^FD${datos.localidad}^FS`
  
  // Agregar entre calles si existe
  if (datos.entreCalles) {
    zpl += `\n^FO50,570^CF0,30^FDEntrecalles:^FS
^FO200,570^CF0,30^FD${datos.entreCalles}^FS`
  }
  
  zpl += `\n^FO50,620^CF0,30^FDTIPO:^FS
^FO250,620^CF0,40^FD${datos.tipoEnvio}^FS`
  ...
  if (datos.tipoEntrega === 'COBRAR' && montoNumero) {
    ...
    zpl += `\n^FO50,670^CF0,40^FDCOBRAR:^FS
^FO250,670^CF0,40^FD${montoFormateado}^FS`
  } else if (datos.tipoEntrega === 'SOLO ENTREGAR') {
    zpl += `\n^FO250,670^CF0,40^FDSOLO ENTREGAR^FS`
  }
  ...
  if (datos.observaciones) {
    const obsTruncadas = datos.observaciones.length > 150 
      ...
      : datos.observaciones
    zpl += `\n^FO50,720^CF0,20^FB700,3,0,L^FDOBSERVACIONES: ${obsTruncadas}^FS`
  }
  ...
  zpl += `\n^FO250,840^BQN,2,8^FD${qrData}^FS
^FO250,1000^CF0,30^FD${codigoSeguimiento}^FS
^XZ`
